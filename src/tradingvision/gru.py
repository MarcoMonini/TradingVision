...
def fold(x: np.ndarray, train: pd.DataFrame, test: pd.DataFrame, seed: int = 0, **kw) -> pd.Series:
...
def main() -> None:
    ...
    out = run(x, df, args.test_start, args.folds, args.seeds, epochs=args.epochs, quiet=not args.verbose)
